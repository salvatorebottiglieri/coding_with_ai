...
const diagnostics = defineDiagnostics({
	...
	codes: {
		...
		DTK0051: { why: "Cannot change the id of a dock. Use register() to add new docks." },
		DTK0052: { why: (p) => `Dock with id "${p.id}" is not registered. Use register() to add new docks.` },
		DTK0053: { why: (p) => `Terminal session with id "${p.id}" already registered` },
		DTK0054: { why: (p) => `Terminal session with id "${p.id}" not registered` },
		DTK0055: { why: (p) => `Command "${p.id}" is already registered` },
		DTK0056: { why: "Cannot change the id of a command. Use register() to add new commands." },
		DTK0057: { why: (p) => `Command "${p.id}" is not registered` }
	}
});
//#endregion
...
var DevToolsCommandsHost = class {
	...
	register(command) {
		if (this.commands.has(command.id)) throw diagnostics.DTK0055({ id: command.id });
		this.commands.set(command.id, command);
		this.events.emit("command:registered", this.toSerializable(command));
		return {
			id: command.id,
			update: (patch) => {
				if ("id" in patch) throw diagnostics.DTK0056();
				const existing = this.commands.get(command.id);
				if (!existing) throw diagnostics.DTK0057({ id: command.id });
	...
	async execute(id, ...args) {
		...
		if (!found) throw diagnostics.DTK0057({ id });
...
var DevToolsDockHost = class {
	...
	register(view, force) {
		if (this.views.has(view.id) && !force) throw diagnostics.DTK0050({ id: view.id });
		this.prepareRemoteRegistration(view);
		this.views.set(view.id, view);
		this.events.emit("dock:entry:updated", view);
		return { update: (patch) => {
			if (patch.id && patch.id !== view.id) throw diagnostics.DTK0051();
			this.update(Object.assign(this.views.get(view.id), patch));
		} };
	}
	update(view) {
		if (!this.views.has(view.id)) throw diagnostics.DTK0052({ id: view.id });
		this.prepareRemoteRegistration(view);
		this.views.set(view.id, view);
		this.events.emit("dock:entry:updated", view);
	}
...
var DevToolsTerminalHost = class {
	...
	*/
	...
	register(session) {
		if (this.sessions.has(session.id)) throw diagnostics.DTK0053({ id: session.id });
	...
	update(patch) {
		if (!this.sessions.has(patch.id)) throw diagnostics.DTK0054({ id: patch.id });
	...
	async startChildProcess(executeOptions, terminal) {
		if (this.sessions.has(terminal.id)) throw diagnostics.DTK0053({ id: terminal.id });
